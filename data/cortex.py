...
class Cortex(DatasetHandler):
    ...
    img_filename = "orig.nii.gz" #"mri.nii.gz"
    ...
    #save the mesh from a subject as template
    def store_specific_template(self, path, n_max_points=45000):
        ...
        #print(len(self.mesh_labels),len(self.mesh_labels[0].vertices)) #1,4
        #input()
        ...
        #print(n_max_points,vertices.shape, vertices_.numpy().shape, faces_.numpy().shape,faces_.numpy()[:5,:])
        structure_1 = trimesh.Trimesh(vertices = vertices_, faces = faces_)#.simplify_quadric_decimation(84030)
        while structure_1.subdivide().vertices.shape[0] < n_max_points:
            structure_1 = structure_1.subdivide()
        print(structure_1.vertices.shape)
        '''
        structure_1 = pv.PolyData(vertices_.numpy(), faces_.numpy())#.triangulate()
        structure_1 = structure_1.decimate_pro(n_max_points, preserve_topology=True)
        structure_1 = trimesh.Trimesh(vertices = structure_1.points, faces = structure_1.triangles)
        '''
        ...
        '''
        ###############1
        vertices = self.mesh_labels[0].vertices[1]
        faces = self.mesh_labels[0].faces[1]
        ...
        faces_ = faces[~(faces == -1).any(dim=1)]
        '''
        structure_2 = pv.PolyData(vertices_.numpy(), faces_.numpy())
        structure_2 = structure_2.decimate_pro(n_max_points, preserve_topology=True)
        structure_2 = trimesh.Trimesh(vertices = structure_2.points, faces = structure_2.triangles)
        '''
        structure_2 = trimesh.Trimesh(vertices = vertices_, faces = faces_)#.simplify_quadric_decimation(84200)
        while structure_2.subdivide().vertices.shape[0] < n_max_points:
            structure_2 = structure_2.subdivide()
        print(structure_2.vertices.shape)
        ###############2
        vertices = self.mesh_labels[0].vertices[2]
        faces = self.mesh_labels[0].faces[2]
        print(vertices.shape)
        ...
        faces_ = faces[~(faces == -1).any(dim=1)]
        '''
        structure_3 = pv.PolyData(vertices_.numpy(), faces_.numpy())
        structure_3 = structure_3.decimate_pro(n_max_points, preserve_topology=True)
        structure_3 = trimesh.Trimesh(vertices = structure_3.points, faces = structure_3.triangles)
        '''
        structure_3 = trimesh.Trimesh(vertices = vertices_, faces = faces_)#.simplify_quadric_decimation(84030)
        while structure_3.subdivide().vertices.shape[0] < n_max_points:
            structure_3 = structure_3.subdivide()
        print(structure_3.vertices.shape)
        ###############3
        vertices = self.mesh_labels[0].vertices[3]
        faces = self.mesh_labels[0].faces[3]
        print(vertices.shape)
        ...
        faces_ = faces[~(faces == -1).any(dim=1)]
        '''
        structure_4 = pv.PolyData(vertices_.numpy(), faces_.numpy())
        structure_4 = structure_4.decimate_pro(n_max_points, preserve_topology=True)
        structure_4 = trimesh.Trimesh(vertices = structure_4.points, faces = structure_4.triangles)
        '''
        structure_4 = trimesh.Trimesh(vertices = vertices_, faces = faces_)#.simplify_quadric_decimation(84200)
        assert structure_1.is_watertight, "Mesh template should be watertight."
        #assert structure_2.is_watertight, "Mesh template should be watertight."
        #assert structure_3.is_watertight, "Mesh template should be watertight."
        #assert structure_4.is_watertight, "Mesh template should be watertight."
        print(f"Template structure has {structure_1.vertices.shape[0]}, {structure_2.vertices.shape[0]},                      {structure_3.vertices.shape[0]},{structure_4.vertices.shape[0]}"
              " vertices.")
        #print(self.mesh_label_names)
        #input()
        template.add_geometry(structure_1, geom_name=self.mesh_label_names[0])
        template.add_geometry(structure_2, geom_name=self.mesh_label_names[1])
        template.add_geometry(structure_3, geom_name=self.mesh_label_names[2])
        template.add_geometry(structure_4, geom_name=self.mesh_label_names[3])
