...
from utils.utils_vox2cortex.graph_conv import (
    ...
    Features2FeaturesResidual_transformer,
    Features2FeaturesResidual_transformer_res,
    Features2FeaturesResidual_transformer_st,
    Features2FeaturesGCN,
    zero_weight_init
)
from utils.utils_vox2cortex.feature_aggregation import (
    aggregate_structural_features,
    mutil_transformer_features_aggregate_from_indices,
...
class GraphDecoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 n_f2f_hidden_layer: int=3): #default=2
        super().__init__()
        ...
        self.graph_conv_first = Features2FeaturesResidual_transformer_st(
            ndims, graph_channels[0], n_f2f_hidden_layer, norm=norm,
            GC=GC, weighted_edges=weighted_edges
        )
        # self.graph_conv_first = Features2FeaturesGCN(
        #     ndims, graph_channels[0], n_f2f_hidden_layer, norm=norm,
        #     GC=GC, weighted_edges=weighted_edges
        # )
        ...
        for i in range(self.num_steps):
            ...
            features_layers.append(mutil_transformer_features_aggregate_from_indices(skip_channels, indices))
            res_blocks = [Features2FeaturesResidual_transformer_st(
            ...
            # res_blocks = [Features2FeaturesGCN(
            #     skip_features_count + self.latent_features_count[i] + add_n,
            #     self.latent_features_count[i+1],
            #     hidden_layer_count=n_f2f_hidden_layer,
            #     norm=norm,
            ...
            for _ in range(n_residual_blocks - 1):
                # res_blocks.append(Features2FeaturesGCN(
                #     self.latent_features_count[i+1],
                #     self.latent_features_count[i+1],
                #     hidden_layer_count=n_f2f_hidden_layer,
                #     norm=norm,
                ...
                # ))
                res_blocks.append(Features2FeaturesResidual_transformer_st(
                    self.latent_features_count[i+1],
                    self.latent_features_count[i+1],
                    hidden_layer_count=n_f2f_hidden_layer,
                    norm=norm,
        ...
        self.feature_layers = nn.ModuleList(features_layers)
        ...
        # raw_sphere_vertices = raw_sphere_vertices[0:2]
        # raw_sphere_faces = raw_sphere_faces[0:2]
        # raw_sphere_vertices = raw_sphere_vertices[2:4]
        # raw_sphere_faces = raw_sphere_faces[2:4]
    ...
    def forward(self, skips):
        ...
        with autocast(enabled=cast):
            ...
            for i, (f2f_res,
                    f2v,
                    agg_indices,
                    do_unpool, feature_block ) in enumerate(zip(
                        self.f2f_res_layers,
                        self.f2v_layers,
                ...
                with autocast(enabled=False):
                    if self.aggregate in ('trilinear', 'bilinear'):
                        ...
                        skipped_features = feature_block(skips, verts_img_co, agg_indices, self.aggregate).view(
                            batch_size, M, V_new, -1)
