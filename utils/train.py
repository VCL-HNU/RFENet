...
class Solver():
    ...
    def compute_loss(self, model, data, iteration) -> torch.tensor:
        ...
        losses = {}
        pred_vertices, pred_faces = model.__class__.pred_to_verts_and_faces(pred)
        pred_vertices = pred_vertices[3].squeeze(1)
        with autocast(self.mixed_precision):
    ...
    def train(self,
        ...
        training_loader = DataLoader(training_set, batch_size=batch_size,num_workers=4,pin_memory=True,
                                     shuffle=True)
        ...
        for epoch in range(start_epoch, n_epochs+1):
            ...
            for iter_in_epoch, data in enumerate(
                        tqdm((training_loader), desc="[{}/{}]".format(epoch, n_epochs))):
                if iteration % self.log_every == 0:
...
def training_routine(hps: dict, experiment_name=None, loglevel='INFO',
    ...
    #print(training_set.n_min_vertices,hps['N_REF_POINTS_PER_STRUCTURE'],type(hps['N_REF_POINTS_PER_STRUCTURE']))
    #input()
...
if __name__ == "__main__":
    ...
    model = ModelHandler["vox2cortex"].value(\
                                        ndims=3,
                                        n_v_classes=2,
                                        n_m_classes=4,
                                        patch_shape=[128, 144, 128],
                                        **model_config)

    input = torch.randn(128, 144, 128).to(device)
