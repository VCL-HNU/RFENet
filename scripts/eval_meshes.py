...
def eval_trt(mri_id, surf_name, eval_params, epoch, device="cuda:0",
             subfolder="meshes"):
...
if __name__ == '__main__':
    ...
    summary_file = os.path.join(
        ...
        f"{eval_params['metrics_csv_prefix']}_{epoch}_summary.csv"
    )
    ...
    for surf_name in SURF_NAMES:
        summary_file = os.path.join(
            eval_params['log_path'],
            f"{surf_name}_{eval_params['metrics_csv_prefix']}_{epoch}_summary.csv"
        )
        mode_to_output_file[mode](np.array(res_surfaces[surf_name]), summary_file)

    print("Done.")

#command python eval_meshes.py  exp_1  82   42016 ''  ad_hd
#python eval_meshes.py  public_model  best   42016 ''  ad_hd